	...
	public class BuildableGameLocation : GameLocation
	{
		...
		public override bool isCollidingPosition(Microsoft.Xna.Framework.Rectangle position, xTile.Dimensions.Rectangle viewport, bool isFarmer, int damagesFarmer, bool glider, Character character, bool pathfinding, bool projectile = false, bool ignoreCharacterRequirement = false)
		{
			if (!glider)
			{
				foreach (Building b in buildings)
				{
					if (b.intersects(position) && (!isFarmer || !b.intersects(Game1.player.GetBoundingBox())))
					{
						if (character != null && character is FarmAnimal)
						{
							Microsoft.Xna.Framework.Rectangle door = b.getRectForAnimalDoor();
							door.Height += 64;
							if (door.Contains(position) && b.buildingType.Value.Contains((character as FarmAnimal).buildingTypeILiveIn.Value))
							{
								continue;
							}
						}
						...
						else if (character != null && character is NPC)
						{
		...
		public void tryToUpgrade(Building toUpgrade, BluePrint blueprint)
		{
			...
			{
				if (toUpgrade.indoors.Value.farmers.Count() > 0)
				{
		...
		public override string doesTileHaveProperty(int xTile, int yTile, string propertyName, string layerName)
		{
			foreach (Building building in buildings)
			{
				if (xTile >= (int)building.tileX && xTile < (int)building.tileX + (int)building.tilesWide && yTile >= (int)building.tileY && yTile < (int)building.tileY + (int)building.tilesHigh)
				{
		...
		{
			if ((!Game1.player.getTileLocation().Equals(tileLocation) || !Game1.player.currentLocation.Equals(this)) && !isTileOccupiedForPlacement(tileLocation) && isTilePassable(new Location((int)tileLocation.X, (int)tileLocation.Y), Game1.viewport) && doesTileHaveProperty((int)tileLocation.X, (int)tileLocation.Y, "NoFurniture", "Back") == null && !caveNoBuildRect.Contains(Utility.Vector2ToPoint(tileLocation)) && !shippingAreaNoBuildRect.Contains(Utility.Vector2ToPoint(tileLocation)))
			{
		...
		public override Point getWarpPointTo(string location)
		{
			foreach (Building building in buildings)
			{
			...
			return base.getWarpPointTo(location);
		}

		public override Warp isCollidingWithDoors(Microsoft.Xna.Framework.Rectangle position)
		{
			for (int i = 0; i < 4; i++)
			{
				...
				foreach (Building building in buildings)
				{
					...
					{
						return getWarpFromDoor(building.getPointForHumanDoor());
					}
				}
			}
			return base.isCollidingWithDoors(position);
		}

		public override Warp getWarpFromDoor(Point door)
		{
			foreach (Building building in buildings)
			{
				_ = (Point)building.humanDoor;
			...
			return base.getWarpFromDoor(door);
		}
